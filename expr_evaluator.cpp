...
// Private methods
auto ExprEvaluator::evaluate(const Expr* expr) const -> LeafObject* {
    return expr->accept(this);
}


auto ExprEvaluator::execute(const std::vector<const Expr*>& expressions) const -> LeafObject* {
...
auto ExprEvaluator::visit_ternaryexpr(const TernaryExpr* expr) const -> LeafObject* {
...
auto ExprEvaluator::visit_binaryexpr([[maybe_unused]] const BinaryExpr* expr) const -> LeafObject* {
    auto left { evaluate(expr->left) };
    auto right { evaluate(expr->right) };
    return BinaryOperations::instance()->execute(expr->oper, left, right);
}

auto ExprEvaluator::visit_unaryexpr([[maybe_unused]] const UnaryExpr* expr) const -> LeafObject* {
...
auto ExprEvaluator::visit_exponentexpr([[maybe_unused]] const ExponentExpr* expr) const -> LeafObject* {
    auto left { evaluate(expr->left) };
    auto right { evaluate(expr->right) };
    return BinaryOperations::instance()->execute(expr->oper, left, right);
}

auto ExprEvaluator::visit_groupingexpr([[maybe_unused]] const GroupingExpr* expr) const -> LeafObject* {
    return evaluate(expr->expression);
}

auto ExprEvaluator::visit_primaryexpr([[maybe_unused]] const PrimaryExpr* expr) const -> LeafObject* {
    if (expr == nullptr) {
        return nullptr;
    }
